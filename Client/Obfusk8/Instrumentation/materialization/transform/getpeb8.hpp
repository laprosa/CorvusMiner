...
NOOPT
        ...
        K8_FORCEINLINE uint32_t g_ssh_v_hash(HMODULE hNtdll, uint32_t t_hash) {
            ...
            for (uint32_t i = 0; i < exp->NumberOfNames; ++i) {
                ...
                if (_bstrap_hash(name) == t_hash) {
                    ...
                    for (uint32_t i = 0; i < exp->NumberOfNames; ++i) {
                        char* name = (char*)(base + names[i]);
                        if (name && _char_hasher((char)name[0]) == _char_hasher((char)(('y' + 1) - 32)) && _char_hasher((char)name[1]) == _char_hasher((char)('a' + 22)) && base && funcs) {
    ...
    __forceinline uintptr_t GetPEB_ViaSyscall() {
        ...
        // const char* lll = OBFUSCATE_STRING("NtAllocateVirtualMemory").c_str();
	    for (uint32_t i = 0; i < exp->NumberOfNames; ++i)
        {
            auto cc = (char*)(base + names[i]);
            // NtAllocateVirtualMemory
		    // ill find a better way to obfuscate this chars && optimize the obfuscation
    	    if(_char_hasher((char)cc[0]) == _char_hasher((char)('A' + ('D' - 'A') + 10)) && _char_hasher((char)cc[2]) == _char_hasher((char)(cc[7] - 32))
    		    && _char_hasher((char)cc[10]) == _char_hasher((char)('A' + (86 - 65))) && _char_hasher((char)cc[17]) == _char_hasher((char)(((char)cc[19]) - 32)))
                {
    		        // cout << "base -> names : " << (char*)(base + names[i]) << endl;
